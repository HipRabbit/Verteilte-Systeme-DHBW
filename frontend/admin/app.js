...
const SERVICES = [
    { key: 'bewertungen', name: 'Bewertungs-Server', port: 3001, url: 'http://localhost:3001', apiUrl: API.bewertungen, icon: 'star-fill', ressource: 'Bewertungen' },
    { key: 'hotels',      name: 'Hotel-Server',      port: 3002, url: 'http://localhost:3002', apiUrl: API.hotels,      icon: 'building',  ressource: 'Hotels' },
    { key: 'fluege',      name: 'Flugverbindung-Server', port: 3003, url: 'http://localhost:3003', apiUrl: API.fluege,  icon: 'airplane',  ressource: 'Flüge' },
    { key: 'mietwagen',   name: 'Mietwagen-Server',  port: 3004, url: 'http://localhost:3004', apiUrl: API.mietwagen,   icon: 'car-front', ressource: 'Mietwagen' }
];
...
function adminBarRender() {
    ...
    if (!session) return; // Sollte nicht passieren wegen Auth-Check
    bar.innerHTML = `
        <div class="user-info-badge">
            <i class="bi bi-person-circle"></i>
            <span>${session.anzeigeName} <span class="badge bg-danger ms-1">Admin</span></span>
        </div>
        <button id="theme-toggle-admin" class="btn btn-outline-light btn-sm" title="Design wechseln" style="border-radius:50px">
            <i class="bi bi-moon-stars" id="admin-theme-icon"></i>
        </button>
        <a href="../user/index.html?from=admin" class="btn btn-outline-light btn-sm">
            <i class="bi bi-eye"></i> User-Ansicht
        </a>
...
async function dashboardLaden() {
    ...
    try {
        const [hotels, fluege, mietwagen, bewertungen] = await Promise.all([
            apiCall(API.hotels).catch(() => []),
            apiCall(API.fluege).catch(() => []),
            apiCall(API.mietwagen).catch(() => []),
            apiCall(API.bewertungen).catch(() => [])
        ]);
        ...
        cards.innerHTML = `
            ...
            <div class="col-md-3 col-sm-6">
                <div class="card stat-card stat-bg-bewertungen">
                    <div class="card-body text-center">
                        <i class="bi bi-star-fill" style="font-size:2rem; color: var(--urlaub-sun)"></i>
        ...
        const hotelsMitAvg = hotels.map(h => {
            const eigeneBewertungen = bewertungen.filter(b => b.hotelId === h._id || b.hotelName === h.name);
            const avg = eigeneBewertungen.length > 0
                ? eigeneBewertungen.reduce((s, b) => s + b.sterne, 0) / eigeneBewertungen.length
                : 0;
            return { ...h, avg, anzahl: eigeneBewertungen.length };
            ...
            topHotels.innerHTML = hotelsMitAvg.map(h => `
                <li class="list-group-item">
                    <div class="top-hotel-item">
                        ...
                        <span class="badge bg-warning text-white fs-6 top-hotel-badge">★ ${h.avg.toFixed(1)} (${h.anzahl})</span>
                    </div>
                </li>
            `).join('');
        }
        ...
        cards.innerHTML = `<div class="col-12"><div class="alert alert-danger">Fehler beim Laden des Dashboards: ${err.message}</div></div>`;
    }
}
...
function statusCountdownTick() {
    _statusSek--;
    const el = document.getElementById('status-countdown');
    if (el) el.textContent = `Aktualisierung in ${_statusSek}s`;
    if (_statusSek <= 0) {
        _statusSek = 60;
        statusLaden();
    }
...
function statusAutoRefreshStart() {
    statusAutoRefreshStop();
    _statusSek = 60;
    const el = document.getElementById('status-countdown');
    if (el) el.textContent = `Aktualisierung in ${_statusSek}s`;
    _statusTimer = setInterval(statusCountdownTick, 1000);
}

function statusAutoRefreshStop() {
    if (_statusTimer) { clearInterval(_statusTimer); _statusTimer = null; }
    const el = document.getElementById('status-countdown');
    if (el) el.textContent = '';
}

function statusAutoRefreshReset() {
    if (_statusTimer) statusAutoRefreshStart();
}
...
async function statusLaden() {
    ...
    if (anzahlOnline === SERVICES.length) {
        ...
        gesamt.innerHTML = `<i class="bi bi-x-circle-fill"></i> <strong>Alle Services offline!</strong> Bitte prüfen, ob die Backend-Server gestartet sind.`;
    } else {
        gesamt.className = 'alert alert-warning mb-0';
        gesamt.innerHTML = `<i class="bi bi-exclamation-triangle-fill"></i> <strong>${anzahlOnline} von ${SERVICES.length} Services online.</strong> Einige Services sind nicht erreichbar.`;
    }
}
...
async function hotelsLaden() {
    ...
    try {
        state.hotels = await apiCall(API.hotels);
        hotelsRender();
    } catch (err) {
...
function hotelNeu() {
    ...
    ['hotel-name','hotel-stadt','hotel-land','hotel-sterne','hotel-preis','hotel-beschreibung','hotel-ausstattung','hotel-bildUrl','hotel-bilder'].forEach(id => document.getElementById(id).value = '');
...
async function hotelSpeichern() {
    ...
    const daten = {
        ...
        bilder:  document.getElementById('hotel-bilder').value.split(',').map(s => s.trim()).filter(Boolean)
    };
...
async function fluegeLaden() {
    ...
    try {
        state.fluege = await apiCall(API.fluege);
        fluegeRender();
    } catch (err) {
...
async function flugSpeichern() {
    ...
    if (daten.preis < 0)
        return fehlerZeige('flug-fehler', 'Preis darf nicht negativ sein.');
...
async function mietwagenLaden() {
    ...
    try {
        state.mietwagen = await apiCall(API.mietwagen);
        mietwagenRender();
    } catch (err) {
        tbody.innerHTML = `<tr><td colspan="6"><div class="alert alert-danger mb-0">Fehler: ${err.message}</div></td></tr>`;
    }
...
function mietwagenNeu() {
    ...
    ['mw-marke','mw-modell','mw-standort','mw-preis','mw-bildUrl','mw-bilder'].forEach(id => document.getElementById(id).value = '');
...
async function mietwagenSpeichern() {
    ...
    const daten = {
        ...
        bilder:  document.getElementById('mw-bilder').value.split(',').map(s => s.trim()).filter(Boolean)
    };
    if (!daten.marke || !daten.modell || !daten.standort)
        return fehlerZeige('mietwagen-fehler', 'Bitte alle Pflichtfelder ausfüllen.');
    if (daten.preisProTag < 0)
        return fehlerZeige('mietwagen-fehler', 'Preis darf nicht negativ sein.');
...
async function bewertungenLaden() {
    ...
    try {
        state.bewertungen = await apiCall(API.bewertungen);
        bewertungenRender();
    } catch (err) {
        tbody.innerHTML = `<tr><td colspan="6"><div class="alert alert-danger mb-0">Fehler: ${err.message}</div></td></tr>`;
    }
...
function bewertungenRender() {
    ...
    if (liste.length === 0) {
        tbody.innerHTML = emptyZeile(6, 'Noch keine Bewertungen vorhanden', 'star');
        return;
    }
...
async function hotelDropdownFuellen(selectedId = '') {
    ...
    try {
        if (state.hotels.length === 0) state.hotels = await apiCall(API.hotels);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const statusBtn = document.getElementById('btn-tab-status');
    if (statusBtn) {
        statusBtn.addEventListener('shown.bs.tab', () => statusAutoRefreshStart());
        statusBtn.addEventListener('hide.bs.tab',  () => statusAutoRefreshStop());
    }
});
